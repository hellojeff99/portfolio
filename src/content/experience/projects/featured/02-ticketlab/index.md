...
![](./image-20260721145349185.png)image-20260721145349185" style="zoom:20%;">
...
동일 좌석에 요청이 집중되는 환경이므로, 충돌 후 실패하는 Optimistic Lock보다 **먼저 접근을 직렬화하는 방식**을 선택

> **선택 이유:** 중복 예약을 막으면서 Optimistic Lock보다 평균 응답이 32.3% 빠름
...
반복되는 좌석 조회를 Redis가 우선 처리하도록 읽기 경로를 단축

> **변경 반영:** 예약 Commit 후 캐시를 제거하고, Redis 장애 시 DB로 Fallback
...
> 복합 인덱스는 SQL을 3.591ms에서 0.352ms로 줄였지만, API 전체에서는 Redis의 개선 폭이 가장 컸음

---

## 4. 최종 설계

![](./image-20260721145423556.png)
...
- 조회 성능: 1,000 VU, VU당 10회 조회, 좌석 100개
